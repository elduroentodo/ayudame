...
export default function AppPage() {
  ...
  if (!business) return <main className="onboarding-page"><form className="onboarding-card onboarding-form" onSubmit={createWorkspace}>
    ...
    <section className="first-content"><p className="eyebrow">Primer conocimiento</p><h2>Agrega el About de tu negocio</h2><label>Escribe el About <span>(o sube un PDF)</span><textarea name="about" value={form.about} onChange={updateForm} placeholder="Qué hacen, para quién, servicios, horarios y preguntas frecuentes…" /></label><label>Primer PDF <span>(opcional)</span><input type="file" accept="application/pdf" onChange={(event) => setFirstPdf(event.target.files?.[0] || null)} /></label></section>
